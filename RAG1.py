...
SYSTEM_PROMPT_RAG = (
    "Tu es un expert et tuteur précis. "
    "Tu dois répondre à la question en te basant EXCLUSIVEMENT sur le contexte fourni. "
    "Si tu trouves les informations pertinentes dans le contexte, synthétise une réponse claire et complète en français. "
    "Si le contexte est insuffisant ou ne contient pas la réponse, réponds simplement : 'Je n'ai pas pu trouver cette information spécifique dans les documents fournis.'"
)
...
RÈGLE ABSOLUE:Une fois que l'outil a renvoyé un résultat, tu DOIS utiliser ce résultat pour formuler ta réponse à l'utilisateur. Ne dis jamais 'Je n'ai pas pu obtenir...' ou 'Je ne peux pas fournir...' si un outil t'a donné des données.
**Synthétise le contenu fourni par la recherche web, même s'il s'agit de résumés de prévisions ou de sources d'information.**
...
3. web_search: UNIQUEMENT pour informations externes NON liées à RTE
   → Exemples: météo, heure, actualités mondiales, événements récents externes
...
def creer_outil_web_search():
    ...
    return Tool.from_function(
        ...
        description="Recherche web en temps réel EXTERNE pour informations NON liées à RTE (Réseau de Transport d'Électricité). "
            "UTILISE CET OUTIL pour la **météo**, l'**heure**, les **actualités générales**, les **prix spot de l'énergie**, et les **certificats d'économies d'énergie (CEE)**. "
            "Pour les prix et CEE, privilégie les requêtes cherchant la 'dernière cotation' ou la 'source officielle'. "
            "La requête de recherche doit être la plus **précise** possible (ex: 'météo Paris', 'prix spot électricité France aujourd'hui')." , 
...
# ────────────────────────── RTE Tension ───────────────────────────────
def creer_outil_rte():
    """Crée l'outil pour les jours de tempo RTE"""
    try:
        from Jourdetension import JourDeTensionRTE, format_reponse_rte 
    except ImportError:
...
def load_documents():
    ...
    if not loaders:
        ...
        print(f"Warning: No .txt or .pdf found inside {DOCS_DIR.absolute()}. RAG will be empty.")
        return []
...
def init_rag_system():
    ...
    if Path(PERSIST_PATH).exists():
        ...
        # Crée un store vide si aucun doc et aucun store existant
        print("No documents, creating empty vector store placeholder.")
...
class SmartAssistant:
    ...
    def determine_route(self, query: str) -> str:
        ...
        tool_keywords = [
            ...
            # Météo / Web (non-RTE)
            'météo', 'meteo', 'température', 'actualité', 'news', 'temps qu\'il fait', 'prix spot', 'cotation', 'marché de l électricité', 'cours',
             'actualite', 'qui est le president', 'dernières nouvelles','heure', 'quelle heure', 'temps actuel','certificats d économies d énergie', 'CEE', 'valeur CEE',
            # RTE Tension (Tempo)
            'tempo', 'couleur tempo', 'jour tempo', 'rte tension', 'jour tempo de tension'
        ]
    ...
    # Fonction pour formater le résultat RAG 
    ...
    async def process(self, user_input: str) -> str:
        ...
        print(f"🚦 Route choisie: {route}")  # Ajouté pour débuguer
